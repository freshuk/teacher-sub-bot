...
# ───────────────────── אייקון בוט ─────────────────────
ICON_PATH = Path("bot_calendar.png")           # שמור את התמונה בשם זה לצד הקובץ
if ICON_PATH.exists():
...
def bot(msg: str):  st.session_state.chat.append(("bot", msg))
def user(msg: str): st.session_state.chat.append(("user", msg))
...
def run_calc():
    ...
    if res == "DAY_OFF":
        bot(
            f"✋ {st.session_state.teacher} בחופש ביום **{st.session_state.day}** – אין צורך בחלופה."
        )
    else:
        ans = (
            f"להלן החלופות למורה **{st.session_state.teacher}** ביום **{st.session_state.day}**:\n"
        )
        for h in range(st.session_state.start, 6 + 1):
            subj, subs = res[h]
            ans += f"\n**🕐 שעה {h}** – {subj}\n"
            if subs is None:
